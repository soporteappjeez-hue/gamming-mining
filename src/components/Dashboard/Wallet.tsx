      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {sortedWallet.length === 0 ? (
          ...
          sortedWallet.map(item => (
            <div key={item.coinId} className="bg-cyber-panel p-3 rounded-lg border border-cyber-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded flex items-center justify-center text-xl" style={{ backgroundColor: '#33333322' }}>
                  {item.coin?.icon ?? '?'}
                </div>
                <div>
              ...
              <div className="text-right">
                <p className="font-bold text-cyber-accent">
                  {item.amount > 1000 ? `${(item.amount / 1000).toFixed(2)}K` : item.amount.toFixed(item.amount < 1 ? 6 : 4)}
                </p>
